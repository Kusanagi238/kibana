...
const getVersion = (collector: OtelCollector): string | undefined =>
  collector.identifying_attributes?.['service.version'] as string | undefined;
    ...
    return [
      {
        ...
        render: (_: string, collector: OtelCollector) => {
          return (
            <a href={getHref('agent_details', { agentId: collector.id })}>
      ...
      },
      {
        ...
        render: (_: unknown, collector: OtelCollector) => getVersion(collector) ?? '—',
      },
      {
        field: 'signals',
              ...
              {signals.map((signal) => {
                ...
                return (
                  <EuiFlexItem grow={false} key={signal}>
                    <EuiBadge color={color[0]}>
                      <EuiText size="xs" color={color[1]}>
                        {signal}
                      </EuiText>
                    </EuiBadge>
                  </EuiFlexItem>
